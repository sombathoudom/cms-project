...
perform_execute() {
  ...
  while IFS= read -r line; do
    ...
    if output=$(run_with_retry gh issue create --repo "$REPO" --title "$title" --milestone "$milestone" "${label_args[@]}" --body-file "$tmpfile" --json number,state --jq '{number:number,state:state}' 2>&1); then
      local number state
      number=$(python -c 'import json,sys; data=json.loads(sys.stdin.read()); print(data.get("number",""))' <<<"$output")
      state=$(python -c 'import json,sys; data=json.loads(sys.stdin.read()); print(data.get("state",""))' <<<"$output")
